...
- resolves the integration by `integrationKey`
- uses provider-facing location fields such as `restaurantId` and `restaurantName`
...
Permanent RestroX restaurant/location identifier.

Used as the primary mapping key between RestroX and Samparka.
...
Human-readable restaurant/location name.
...
Samparka outlet ID that the RestroX restaurant should map to.
...
```text
store has zero outlets
```

because store-level mappings are supported.

### Single-Location Example

For merchants without outlets, RestroX can sync a single restaurant without an `outletId`:
...
- no `outletId` is required
- the `restaurantId` maps directly to the store-level location
...
- `outletId`
- or successful outlet matching during sync review
...
- `restaurantId` is the preferred field when you already know the RestroX location identifier
- `restaurantId` is the authoritative RestroX restaurant/location mapping key
- `restaurantId` must be the RestroX restaurant identifier, not a Samparka internal identifier
- top-level aliases are also accepted for compatibility:
  - `locationId`
  - `branchId`
  - `id`
- the payload may still supply the location identifier through:
  ...
  - `outlet_id`
  - `branch_id`
- the payload may still supply the location name through:
  - `restaurantName`
  - `external_location_name`
  - `location_name`
  - `branch_name`
...
2. determine the requested location identifier from `restaurantId` or supported aliases
3. reject conflicting top-level and payload identifiers
4. resolve the mapped location by integration plus the internally stored `external_location_id`
...
- `503 RestroX partner API is not configured` — the partner key environment variable is not set on the Samparka server
- `401 Unauthorized partner request` — wrong or missing partner key
- `404 Invalid Integration Key` — the `integrationKey` body field does not match an active integration
- `400 restaurantId does not match the payload location identifier`
- `400 A location identifier is required. Provide restaurantId or a payload location field.`
