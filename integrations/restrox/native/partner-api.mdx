...
flowchart TD
  ...
  F --> G["Resolve location + webhook token"]
  G --> H["handleTokenEvent()"]
  H --> I["Canonical ingress"]
  I --> J["Merchant reaches ready state"]
```
...
- syncs the supplied locations into `posIntegrationLocation`
...
- `locationId` is not a Samparka Mongo `_id`
...
5. require a stored `webhook_token`
6. inject the canonical `external_location_id` into the synthetic payload
7. invoke `handleTokenEvent()`
8. continue through the same canonical token-routed ingress used by production webhook traffic

Downstream canonical ingress stages:
...
- event processing and loyalty path

Verified behavior:

- does not use `integration.api_key`
- does not send `x-api-key`
- does not call `handleEvent()`
- updates `integration_key_last_used_at` after the token-routed ingress call returns
...
- canonical ingress validation failures such as:
...
- test sale received through the canonical token-routed ingress
...
- use the native test-sale path to validate the same token-routed ingress used by production webhook traffic
