...
```mermaid
flowchart TD
  A["RestroX connect + sync"] --> B["Customer lookup"]
...
- `created`: loyalty processing may create a customer internally according to Samparka identity rules
...
- If customer search is working but loyalty is still missing, inspect location state and event validation next.
