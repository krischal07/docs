...
See [Event Types](../event-types) for the full list of accepted aliases.
...
If an unrecognized `event_type` value is received, Samparka maps it to `sale.completed` and processes it as a completed sale. No error is returned to the sender. Use the canonical event type names in [Event Types](../event-types) to ensure predictable processing.
