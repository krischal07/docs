...
description: Canonical cashier, customer, RestroX POS, and Samparka workflow for loyalty participation and loyalty earning during a sale.
sidebarTitle: Loyalty Award Flow
---

# Customer Loyalty Award Flow

This page is the canonical workflow guide for loyalty awarding in the RestroX native integration.
...
RestroX identifies the customer, associates that identity to the sale, and submits the completed sale event.

Samparka calculates everything else.

### Customer Ownership Boundaries

RestroX POS does not expose customer creation or update actions to cashiers.
...
RestroX performs customer search through the read-only partner customer API:
...
- RestroX should display the returned customer information as read-only

This keeps customer identity owned by Samparka and aligns with the read-only customer APIs.

## Customer Association Model

This is the key implementation rule for RestroX engineers:
...
When a customer is found, RestroX should associate the returned Samparka customer ID with the in-progress bill or sale context.

The phone number is the lookup key. The Samparka customer ID is the identity RestroX should retain for the transaction when available.
...
RestroX then submits the completed sale using the associated customer identity.
...
RestroX does not perform loyalty calculations.
