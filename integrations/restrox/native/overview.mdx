...
# RestroX Native Integration

RestroX can connect to Samparka through a native onboarding flow that sits above Samparka's existing webhook processing engine.
...
- resolve customers consistently
...
The native integration uses partner APIs for onboarding and customer operations, while the existing webhook engine remains the event transport and processing layer.
...
<Steps>
  ...
  <Step title="Implement Customer Resolution">
    Use Customer Identity and Customer API to make customer lookup, create, and upsert behavior deterministic.
  </Step>
