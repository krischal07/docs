...
| `400 Invalid purchase QR request` on purchase QR | Missing/empty `items`, non-positive `amount`, or bad `currency` | Inspect the request body against the [Payload Reference](/integrations/system/payload-reference). | Correct the fields and resend. |

    <Columns cols={2}>
      <Card title="Request / Response Contract" icon="code" href="/integrations/system/purchase-qr/request-response">
        Full error codes and status lifecycle.
      </Card>
