---
title: System Integration Audit Notes
description: Root-level audit summary for the current System contract.
...
System is an adapter-based webhook ingress. `src/integrations/system/providers/index.js` is the registry; it contains **Blanxer** and **RestroX**, each exposing `validate`, `parser.parse`, and `mapper.map`. `bootstrapProviders.js` seeds Provider records for Samparka, Blanxer, and RestroX. Provider records declare capabilities; the code rejects webhook use when the provider is not active/capable.

Public receipt endpoints are mounted twice: `POST /integrations/system/:provider/events` and `/webhook/:provider/events`; the token routes are `POST /integrations/system/restrox/:token`, `/blanxer/:token`, and `/:provider/:token`. The generic legacy provider route returns 410 unless `SYSTEM_LEGACY_PROVIDER_ROUTE_ENABLED=true`. The merchant-management API is under `/api/system-integrations`; administration/replay surfaces are mounted under `/api/admin/system` and provider administration under `/api/admin/system/providers`. Partner lifecycle endpoints are `/api/partners/:provider/{connect,disconnect,test-sale}` (Bearer provider API key) and RestroX has `/api/partners/restrox/{connect,sync-locations,test-sale}`.
...
1. `SystemWebhookController._handle` generates a request ID, checks the named adapter, derives an idempotency key, and immediately writes a `RawWebhookEvent`. This happens before authentication, so failures are auditable (except an unknown provider, which is rejected before the write).
2. It checks provider capability, validates the request with the provider adapter, and resolves merchant routing. Token routes resolve `webhook_token` through `SystemIntegrationLocation`; legacy routes resolve `(provider, x-api-key)` against `MerchantIntegration`/`SystemIntegration`.
3. The resolved integration supplies `store_id`. Outlet-owned integrations use their bound `outlet_id`; store-owned integrations map the provider external location ID through `SystemIntegrationLocation`. Missing, stale, disabled, non-participating, or unmapped locations produce an `InternalEvent` in `blocked_unmapped_location`, which is acknowledged but intentionally not processed.
...
| Outlet | Outlet-owned integration binding, otherwise `SystemIntegrationLocation` external-location mapping and validity evaluation. |
...
| Duplicate | `ProcessedEvent` unique index `(provider,system_integration_id,idempotency_key)`. Key is event type + provider ID, or a SHA-256 fallback of selected transaction fields. |
...
| Retry | retryable processor failures can be replayed by admin reprocess/replay services. The controller itself is synchronous; it does not enqueue System processing. |
...
`RawWebhookEvent` is immutable payload audit storage with received/validated/normalized/processed/failed/duplicate status. `ProcessedEvent` is the durable reservation/dedup record (`reserved`, `processed`, `failed`). `InternalEvent` is the normalized domain input and processing/replay record. `EventProcessorState` stores a per-processor snapshot. `MerchantIntegration`/`SystemIntegration` and `SystemIntegrationLocation` represent connection and mapping; `Provider`/`ProviderApiKey` represent supported vendors and partner credentials. The downstream collections are `PurchaseTransaction`, `PurchaseItem`, `LoyaltyLedger`, `Customer`, and for L5 `CustomerOutletPoints` plus `L5PointsTransaction`.
...
|---|---|
| `/integrations/system/:provider/events` | Legacy adapter webhook; provider validator and `x-api-key`, feature-flagged. |
| `/integrations/system/{restrox,blanxer}/:token`, `/:provider/:token` | Token-routed adapter webhooks; token resolves integration/location. |
| `/api/v1/integrations/purchases` | `integrationAuthMiddleware('submitPurchase')`; generic canonical purchase command. |
| `/api/system-integrations/*` | `adminOrStoreMiddleware`; CRUD, locations, tokens/keys, status, verification, onboarding, sync/activity/share. |
| `/api/admin/system/{raw-webhook-events,internal-events,system-location-mappings,metrics,system-reconciliation}` | admin middleware; inspection and replay/reprocess, not live System ingress. |
| `/api/admin/system/providers/*` | admin middleware; provider catalogue, provider keys, provider integrations/activity. |
...
| `/integrations/system/:provider/:token/purchase-qr` | Token-routed Purchase QR; creates scannable checkout session for cash sales. Returns QR link in same response. |

### Endpoint catalog — Communication tab

The endpoint catalog groups System endpoints into **App** (webhook delivery, partner lifecycle, customer lookup) and **Communication** (Purchase QR) tabs. The Communication tab contains the Purchase QR endpoint and links to its overview, request/response contract, integration guide, and testing docs.

## Purchase QR — items-based flow

The Purchase QR endpoint was refactored to remove `bill_id` and `customer_phone` from the request. The System now sends `amount`, `currency`, and `items` (product items with `name`, `qty`, `price`).
...
All Purchase QR documentation now uses generic `:provider` path params and `{provider}` in examples instead of hardcoding `blanxer` or `restrox`. The endpoint works with any registered System provider.

## Text sequence diagram

```text
System -> SystemWebhookController: POST token webhook
...
Controller --> System: acknowledgement
```

## Important code units

- `SystemWebhookController._handle`: inbound orchestration and acknowledgement.
...
- `IntegrationEventService` and `SystemWebhookService`: raw audit, reservation, and event persistence.
...
- `replayService`, `reprocessBlockedEventService`: operator recovery; no automatic System queue exists.
