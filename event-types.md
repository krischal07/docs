---
title: Event Types
description: Root-level event type summary for System.
...
<Tabs>
  ...
  <Tab title="Communication">
    ...
    The Purchase QR endpoint does not use webhook event types. Instead, the System sends a `POST` request with `amount`, `currency`, and `items` to create a checkout session. The customer then completes the loyalty flow by scanning the QR on their receipt.

    | Concern | App (webhook events) | Communication (Purchase QR) |
    | ------- | -------------------- | --------------------------- |
    | Entry point | `order.completed`, `refund.created`, `order.voided` | `POST /integrations/system/{provider}/purchase-qr` |
